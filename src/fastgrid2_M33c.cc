...
double _fastgrid2M33c_search(Matrix<double,Row>& X, Matrix<double,Row>& Y, vector<double>& w, bool wAllOne, 
    ...
    Matrix<double,Row>& XcusumR, vector<double>& YcusumR, vector<double>& WcusumR, 
    vector<double>& xcusumR, vector<double>& x2cusumR, vector<double>& x3cusumR, vector<double>& x4cusumR, vector<double>& x5cusumR, 
    ...
    double* logliks)
{
    ...
    XcusumR(n-1,_) = X(n-1,_)* w[n-1];    
    YcusumR[n-1] = Y(n-1)* w[n-1];        
    WcusumR[n-1] = w[n-1];              
    xcusumR[n-1]    = x_cpy[n-1] * w[n-1]; // xcusumR is the last columnn of XcusumR, but perhaps better to be its own variable
    x2cusumR[n-1]   = pow(x_cpy[n-1],2) * w[n-1];        
    x3cusumR[n-1]   = pow(x_cpy[n-1],3) * w[n-1];        
    x4cusumR[n-1]   = pow(x_cpy[n-1],4) * w[n-1];        
    x5cusumR[n-1]   = pow(x_cpy[n-1],5) * w[n-1];        
    xrcusumR[n-1]   = x_cpy[n-1] * Y(n-1)* w[n-1];        
    x2rcusumR[n-1]  = pow(x_cpy[n-1],2) * Y(n-1)* w[n-1];        
    xBcusumR(n-1,_) = x_cpy[n-1] * X(n-1,_)* w[n-1];    
    x2BcusumR(n-1,_)= pow(x_cpy[n-1],2) * X(n-1,_)* w[n-1];    
    for (i=n-2; i>=0; i--) {
        XcusumR(i,_)  = XcusumR(i+1,_)    + X(i,_)* w[i]; 
        YcusumR[i]    = YcusumR[i+1]      + Y(i)* w[i]; 
        WcusumR[i]    = WcusumR[i+1]      + w[i]; 
        xcusumR[i]    = xcusumR[i+1]      + x_cpy[i]* w[i]; 
        x2cusumR[i]   = x2cusumR[i+1]     + pow(x_cpy[i],2)* w[i]; 
        x3cusumR[i]   = x3cusumR[i+1]     + pow(x_cpy[i],3)* w[i]; 
        x4cusumR[i]   = x4cusumR[i+1]     + pow(x_cpy[i],4)* w[i]; 
        x5cusumR[i]   = x5cusumR[i+1]     + pow(x_cpy[i],5)* w[i]; 
        xrcusumR[i]   = xrcusumR[i+1]     + x_cpy[i]* Y(i)* w[i]; 
        x2rcusumR[i]  = x2rcusumR[i+1]    + pow(x_cpy[i],2)* Y(i)* w[i]; 
        xBcusumR(i,_) = xBcusumR(i+1,_)   + x_cpy[i]* X(i,_)* w[i]; 
        x2BcusumR(i,_)= x2BcusumR(i+1,_)  + pow(x_cpy[i],2)* X(i,_)* w[i]; 
    }
    ...
    if(wAllOne) {
        ...
        VV(0,2)=0; for (i=0; i<n; i++) VV(0,2) += pow(X(i,p-1),4);
        VV(1,2)=0; for (i=0; i<n; i++) VV(1,2) += pow(X(i,p-1),5);
        VV(2,0)=VV(0,2);
        VV(2,1)=VV(1,2);
    ...
    if(wAllOne) {
        VV(3,3)=0; for (i=0; i<n; i++) VV(3,3) += pow(X(i,p-1),6); 
        VV(0,3)=0; for (i=0; i<n; i++) VV(0,3) += pow(X(i,p-1),4);
        VV(1,3)=0; for (i=0; i<n; i++) VV(1,3) += pow(X(i,p-1),5);
        VV(3,0)=VV(0,3);
        VV(3,1)=VV(1,3);
        
        Vr(3,0)=0; for (i=0; i<n; i++) Vr(3,0) += pow(X(i,p-1),3)*Y(i); 
        
        for (j=0; j<p-1; j++) {
            VB(3,j)=0; for (i=0; i<n; i++) VB(3,j) += pow(X(i,p-1),3) * X(i,j); // remember the first p-1 column of X is B after _preprocess()
        }
    } else {
    }
    ...
    for(tt=0; tt<nThresholds; tt++) { 
        ...
        if(tt>0) {
            ...
            double c7=-2*d*xcusum[n-1] + 2*n*d*e + n * pow(d,2); 
            double c8=-4*d*x3cusum[k-1] + (6*d*e+3*d2+3*d*d)*x2cusum[k-1] - (3*d*e*e+3*d2*e+d3+3*d*d2)*xcusum[k-1] + k*d*e*e*e + k*d3*e + k*d*d3;
            double c9=-5*d*x4cusum[k-1] + (12*d*e+4*d2+6*d*d)*x3cusum[k-1] - (9*d*e*e+9*d2*e+d3+9*d*d2)*x2cusum[k-1] + (2*d*e*e*e+6*d2*e*e+2*d3*e+2*d*d3+3*d2*d2)*xcusum[k-1] - (k*d2*e*e*e + k*d3*e*e + k*d2*d3); 
            double c10=-6*d*x5cusum[k-1] + (18*d*e+6*d2+9*d*d)*x4cusum[k-1] - (18*d*e*e+18*d2*e+2*d3+18*d*d2)*x3cusum[k-1] + (6*d*e*e*e+18*d2*e*e+6*d3*e+6*d*d3+9*d2*d2)*x2cusum[k-1] - (6*d2*e*e*e+6*d3*e*e+6*d2*d3)*xcusum[k-1] + (2*k*d3*e*e*e + k*d3*d3); 
                                        
            double c11= -3*d*x2cusum[n-1] + (4*d*e+d2+2*d*d)*xcusum[n-1] -(n*d*e*e + n*d2*e + n*d*d2);
            double c12= -4*d*x3cusumR[k] + (6*d*e+3*d2+3*d*d)*x2cusumR[k] - (3*d*e*e+3*d2*e+d3+3*d*d2)*xcusumR[k] + (n-k)*d*e*e*e + (n-k)*d3*e + (n-k)*d*d3;
            double c13= -4*d*x3cusum[n-1] + (8*d*e+2*d2+4*d*d)*x2cusum[n-1] - (4*d*e*e+4*d2*e+4*d*d2)*xcusum[n-1] + 2*n*d2*e*e + n*d2*d2;
            double c14= -5*d*x4cusumR[k] + (12*d*e+4*d2+6*d*d)*x3cusumR[k] - (9*d*e*e+9*d2*e+d3+9*d*d2)*x2cusumR[k] + (2*d*e*e*e+6*d2*e*e+2*d3*e+2*d*d3+3*d2*d2)*xcusumR[k] - ((n-k)*d2*e*e*e + (n-k)*d3*e*e + (n-k)*d2*d3); 
            double c15= -6*d*x5cusumR[k] + (18*d*e+6*d2+9*d*d)*x4cusumR[k] - (18*d*e*e+18*d2*e+2*d3+18*d*d2)*x3cusumR[k] + (6*d*e*e*e+18*d2*e*e+6*d3*e+6*d*d3+9*d2*d2)*x2cusumR[k] - (6*d2*e*e*e+6*d3*e*e+6*d2*d3)*xcusumR[k] + (2*(n-k)*d3*e*e*e + (n-k)*d3*d3); 
            ...
            Vr[0] +=   -d * Ycusum[n-1];
            Vr[1] += -2*d * xrcusum[n-1] + d2 * Ycusum[n-1] ;
            Vr[2] += -3*d * x2rcusum[k-1] +3*d2 * xrcusum[k-1] -d3 * Ycusum[k-1] ;
            Vr[3] += -3*d * x2rcusumR[k]  +3*d2 * xrcusumR[k]  -d3 * YcusumR[k] ;
            
            for (j=0; j<p-1; j++) {
                VB(0,j) +=   -d * Xcusum(n-1,j); 
                VB(1,j) += -2*d * xBcusum(n-1,j) + d2 * Xcusum(n-1,j); 
                VB(2,j) += -3*d * x2Bcusum(k-1,j) +3*d2 * xBcusum(k-1,j) -d3 * Xcusum(k-1,j); 
                VB(3,j) += -3*d * x2BcusumR(k ,j) +3*d2 * xBcusumR(k ,j) -d3 * XcusumR(k ,j); 
            }
...
//    Matrix<double,Row,Concrete> Xcpy(X), Ycpy(Y); // debug use
    ...
    Matrix <double,Row,Concrete> XcusumR (n, p), xBcusumR (n, p), x2BcusumR (n, p);
    vector<double> Ycusum(n), Wcusum(n), xcusum (n), x2cusum (n), x3cusum (n), x4cusum (n), x5cusum (n), xrcusum (n), x2rcusum (n); 
    vector<double> YcusumR(n), WcusumR(n), xcusumR(n), x2cusumR(n), x3cusumR(n), x4cusumR(n), x5cusumR(n), xrcusumR(n), x2rcusumR(n); 
    ...
    // a single search
        ...
        _fastgrid2M33c_search(X, Y, W, wAllOne, thresholdIdx, skipping, thresholds, 
                             n, p, nThresholds,
                             Xcusum, Ycusum, Wcusum, xcusum, x2cusum, x3cusum, x4cusum, x5cusum, xrcusum, x2rcusum, xBcusum, x2Bcusum, 
                             XcusumR, YcusumR, WcusumR, xcusumR, x2cusumR, x3cusumR, x4cusumR, x5cusumR, xrcusumR, x2rcusumR, xBcusumR, x2BcusumR, 
//                             Xcpy, Ycpy, // debug use
                             logliks); 
...
//            for (i=0; i<n; i++) { Xb(i,_)=X(i,_); Yb(i)=Y(i); Wb[i]=W[i]; } // debug use, can be used to compare with non-boot
            ...
            e_hat = _fastgrid2M33c_search(Xb, Yb, Wb, wAllOne, thresholdIdx, skipping, thresholds, 
                                 n, p, nThresholds,
                                 Xcusum, Ycusum, Wcusum, xcusum, x2cusum, x3cusum, x4cusum, x5cusum, xrcusum, x2rcusum, xBcusum, x2Bcusum, 
                                 XcusumR, YcusumR, WcusumR, xcusumR, x2cusumR, x3cusumR, x4cusumR, x5cusumR, xrcusumR, x2rcusumR, xBcusumR, x2BcusumR, 
//                             Xcpy, Ycpy, // debug use
            ...
            for (i=0; i<n; i++) { //note that index need to -1 to become 0-based
                ...
                // (x-e)-^3
                if(X(index[i]-1,p-1)<e_hat) Xbreg(i,p+1)=pow(Xbreg(i,p-1),3); else Xbreg(i,p+1)=0;
                // (x-e)+^3
                if(X(index[i]-1,p-1)>e_hat) Xbreg(i,p+2)=pow(Xbreg(i,p-1),3); else Xbreg(i,p+2)=0;
            }     
