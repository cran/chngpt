    ...
    double* logliks)
{
    ...
    VV(0,1)=VV(0,3)=VV(1,0)=VV(1,2)=VV(2,1)=VV(2,3)=VV(3,0)=VV(3,2)=0;
    ...
    VV(1,1)=0; for (i=0; i<n; i++) VV(1,1) += pow(x[i],2)* w[i]* w[i]; 
    VV(3,3)=0; for (i=0; i<n; i++) VV(3,3) += pow(x[i],4)* w[i]* w[i]; 
    VV(1,3)=0; for (i=0; i<n; i++) VV(1,3) += pow(x[i],3)* w[i]* w[i];
    VV(3,1)=VV(1,3);
    
    Vr(1,0)=0; for (i=0; i<n; i++) Vr(1,0) += x[i]*r(i)* w[i]; 
    Vr(3,0)=0; for (i=0; i<n; i++) Vr(3,0) += pow(x[i],2)*r(i)* w[i]; 
    
    for (j=0; j<p-1; j++) {
        VB(1,j)=0; for (i=0; i<n; i++) VB(1,j) += x[i] * B(i,j)* w[i]; // remember the first p-1 column of B is B after _preprocess()
        VB(3,j)=0; for (i=0; i<n; i++) VB(3,j) += pow(x[i],2) * B(i,j)* w[i]; // remember the first p-1 column of B is B after _preprocess()
    }
    ...
    for(tt=0; tt<nThresholds; tt++) { 
        ...
        if(tt>0) {
            ...
            VV(0,0) += -2*delta*xcusum[k-1] + 2*Wcusum[k-1]*delta*e + Wcusum[k-1] * pow(delta,2); 
            VV(2,2) += -4*delta*x3cusum[k-1] + 2*(2*pow(delta,2)+4*delta*e+s)*x2cusum[k-1] - (4*delta*s+4*delta*pow(e,2)+4*e*s)*xcusum[k-1] + Wcusum[k-1]*s*s + 2*Wcusum[k-1]*s*pow(e,2); 
            VV(0,2) += -3*delta*x2cusum[k-1] + (2*pow(delta,2)+4*delta*e+s) * xcusum[k-1] - Wcusum[k-1]*delta*s - Wcusum[k-1]*s*e - Wcusum[k-1]*delta*pow(e,2); 
            VV(2,0) = VV(0,2);
            Vr[0] += -delta * rcusum[k-1];
            Vr[2] += -2*delta * xrcusum[k-1] + s * rcusum[k-1] ;
            for (j=0; j<p-1; j++) {
                VB(0,j) +=   -delta * Bcusum(k-1,j); 
                VB(2,j) += -2*delta * xBcusum(k-1,j) + s * Bcusum(k-1,j); 
            }

            // hinge part
            k  = n-k; // the current threshold is the kth largest 
            VV(1,1) += -2*delta*xcusumR[n-k] + 2*WcusumR[n-k]*delta*e + WcusumR[n-k] * pow(delta,2); 
            VV(3,3) += -4*delta*x3cusumR[n-k] + 2*(2*pow(delta,2)+4*delta*e+s)*x2cusumR[n-k] - (4*delta*s+4*delta*pow(e,2)+4*e*s)*xcusumR[n-k] + WcusumR[n-k]*s*s + 2*WcusumR[n-k]*s*pow(e,2); 
            VV(1,3) += -3*delta*x2cusumR[n-k] +   (2*pow(delta,2)+4*delta*e+s) *xcusumR[n-k] -  WcusumR[n-k]*delta*s - WcusumR[n-k]*s*e - WcusumR[n-k]*delta*pow(e,2); 
            VV(3,1) = VV(1,3);
            Vr[1] += -delta * rcusumR[n-k];
            Vr[3] += -2*delta * xrcusumR[n-k] + s * rcusumR[n-k];
            for (j=0; j<p-1; j++) {
                VB(1,j) +=   -delta * BcusumR (n-k,j); 
                VB(3,j) += -2*delta * xBcusumR(n-k,j) + s * BcusumR(n-k,j); 
            }                        
        }
