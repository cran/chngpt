...
//    	}
//        if(skipping) {
//            xrcusum[n-1]   = x_cpy[n-1]*Y(n-1)   * w[n-1]; 
//            xBcusum(n-1,_) = x_cpy[n-1]*X(n-1,_) * w[n-1]; 
//            for (i=n-2; i>=0; i--) {
...
//        for(i=thresholdIdx[0]; i<n; i++) X(i,p-1)=X(i,p-1)-thresholds[0]; 
    ...
    for(tt=0; tt<nThresholds; tt++) { 
        ...
        if(tt>0) {
            ...
            if (!isUpperHinge) {
//                iX = thresholdIdx[tt]-1; 
//                k  = n-thresholdIdx[tt]+1; // the current threshold is the kth largest 
//                
//                vv -= 2 * delta * (Xcusum(iX,p-1) - k*thresholds[tt-1]) -  k * pow(delta,2); 
//                vr -= delta * Ycusum[iX];
//                for (j=0; j<p-1; j++) vB[j] -= delta * Xcusum(iX,j); 
//
//               	if(skipping) {
//                    // for thinned thresholds
//                    skipped = thresholdIdx[tt]-1 - thresholdIdx[tt-1];
//                    if (skipped>0) {
//                        vv -= xBcusum(thresholdIdx[tt-1],p-1)- xBcusum(thresholdIdx[tt]-1,p-1) 
//                              - 2*thresholds[tt-1]*(Xcusum(thresholdIdx[tt-1],p-1)- Xcusum(thresholdIdx[tt]-1,p-1))
//                              + skipped * pow(thresholds[tt-1],2);
//                        vr -= xrcusum[thresholdIdx[tt-1]]- xrcusum[thresholdIdx[tt]-1]
//                              - thresholds[tt-1]*(Ycusum[thresholdIdx[tt-1]]- Ycusum[thresholdIdx[tt]-1]);
//                        for (j=0; j<p-1; j++) vB[j] -= xBcusum(thresholdIdx[tt-1],j)- xBcusum(thresholdIdx[tt]-1,j)
//                                                       - thresholds[tt-1]*(Xcusum(thresholdIdx[tt-1],j)- Xcusum(thresholdIdx[tt]-1,j)); 
//                    }
//    //                // a more straightforward implementation
//    //                for (m=thresholdIdx[tt-1]; m<thresholdIdx[tt]-1; m++) {
//    //                    d = x_cpy[m]-thresholds[tt-1]; //PRINTF("d %f\n",d);
