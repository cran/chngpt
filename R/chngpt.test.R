...
chngpt.test = function(formula.null, formula.chngpt, data, 
    ...
    if (!has.itxn) interaction.method=""     
    myprint(z.1.name, setdiff(colnames(tmp), colnames(Z))[1])
...
#    isSymmetric(A.h)
    ...
    # likelihood ratio statistics
        ...
        for (m in 1:M) {
            ...
            fit.a=keepWarnings(glm(update(formula.null, as.formula("~.+x.gt.e*"%+%z.1.name)),  data=data, family="binomial", weights=prob.weights))
            if(length(fit.a$warning)!=0) return (NA) else fit.a=fit.a$value
    ...
    res=list()
    res$chngpts=chngpts    
    if (interaction.method %in% c("lr.pastor","lr.mc","score.power","score")) {
        res$TT=QQ
        res$statistic=Q.max
